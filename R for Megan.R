...
standards<-lion%>%
  filter(calibrate == "standard") %>% 
  summarize(
    meansalmonC = mean(standard_c, na.rm = TRUE),
    sdsamlmonC= sd(standard_c, na.rm = TRUE),
    meansalmonN = mean(standard_n, na.rm = TRUE),
    sdsamlmonN= sd(standard_n, na.rm = TRUE))
